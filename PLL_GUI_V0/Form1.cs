...
sort
根据inclk计算出的所有n,m,c0m,c1m,e0m的组合，经过排序与vco筛选

sort2
sort中去除了c0m,c1m,e0m的所有因子（较小值）
占空比值发生改变，应该从这里重新筛选
...
namespace PLL
{
    public partial class Form1 : Form
    {
        ...
        static double vco_min = 300.0;
        static double vco_max = 700.0;

        static double vco_minF = 400.0;
        static double vco_maxF = 800.0;

        static int Nmax = 25;//N最大值
        static int Mmax = 64;//N最大值
        static int NCM = 1600;//N*c0_m
        static int NCML = 768;//N*c0_m

        static int NmaxF = 9;//N最大值
        static int MmaxF = 31;//N最大值
        static int NCMF = 279;//N*c0_m
        ...
        */
        ...
        /*
        //画图相关定义开始
        private Color color = Color.DarkGreen;  // 前景色
        Point startPoint = new Point(50, 50);  // pll图的起点位置
        Font font = new Font("Times New Roman", 8); // 图中字体的大小
        //画图相关定义结束
        */
        ...
        //    //pictureBox.BackColor = Color.FromArgb(206, 232, 200);
        //    DrawPLL();
        //    pictureBox.Refresh();
        //}
        ...
        public Form1(string saveFileName)
        {
        ...
        /*
        private void DrawPLL()
        {
            Pen pen = new Pen(color);
            Graphics g = Graphics.FromImage(pictureBox.Image);
            Rectangle rect = new Rectangle(startPoint.X - 50, startPoint.Y - 50, 300, 170);
            g.FillRectangle(Brushes.White, rect);
            g.DrawRectangle(pen, rect);

            float width = g.MeasureString(moduleName, font).Width;
            g.DrawString(moduleName, new Font("Times New Roman", 12), new SolidBrush(color), startPoint.X + 100 - width / 2, startPoint.Y - 42);

            g.DrawRectangle(Pens.Black, startPoint.X, startPoint.Y, 200, 110);

            // 画输入端
            Point loc = new Point(startPoint.X, startPoint.Y);
            loc.Y += 17;
            g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X - 50, loc.Y);
            g.DrawString("inclk", font, Brushes.Black, loc.X - 47, loc.Y - 13);

            //if (fbinRadioBtn.Checked)
            //{
            //    loc.Y += 17;
            //    g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X - 50, loc.Y);
            //    g.DrawString("fbin", font, Brushes.Black, loc.X - 47, loc.Y - 13);
            //}

            if (CB_pllena.Checked)
            {
                loc.Y += 17;
                g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X - 50, loc.Y);
                g.DrawString("pllena", font, Brushes.Black, loc.X - 47, loc.Y - 13);
            }

            if (CB_areset.Checked)
            {
                loc.Y += 17;
                g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X - 50, loc.Y);
                g.DrawString("areset", font, Brushes.Black, loc.X - 47, loc.Y - 13);
            }

            if (CB_pfd_load.Checked)
            {
                loc.Y += 17;
                g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X - 50, loc.Y);
                string sig = "pfdena";
                ...
                g.DrawString(sig, font, Brushes.Black, loc.X - 47, loc.Y - 13);
            }
            ...
            if (CB_c0.Checked)
            {
                loc.Y += 17;
                g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X + 50, loc.Y);
                g.DrawString("c0", font, Brushes.Black, loc.X + 47 - g.MeasureString("c0", font).Width, loc.Y - 13);
            }
            if (CB_c1.Checked)
            {
                loc.Y += 17;
                g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X + 50, loc.Y);
                g.DrawString("c1", font, Brushes.Black, loc.X + 47 - g.MeasureString("c1", font).Width, loc.Y - 13);
            }
            if (CB_e0.Checked)
            {
                loc.Y += 17;
                g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X + 50, loc.Y);
                g.DrawString("e0", font, Brushes.Black, loc.X + 47 - g.MeasureString("e0", font).Width, loc.Y - 13);
            }
            if (CB_locked.Checked)
            {
                loc.Y += 17;
                g.DrawLine(Pens.Black, loc.X, loc.Y, loc.X + 50, loc.Y);
                g.DrawString("locked", font, Brushes.Black, loc.X + 47 - g.MeasureString("locked", font).Width, loc.Y - 13);
            }

            // 在pll图的内部画各个输出端的频率相位等信息
            g.DrawString("inclk frequency: " + TB_inclk.Text + "MHZ", font, new SolidBrush(color), startPoint.X + 7, startPoint.Y + 11);
            g.DrawString("Operation Mode: " + operation_mode, font, new SolidBrush(color), startPoint.X + 7, startPoint.Y + 27);

            loc.X = startPoint.X + 8;
            loc.Y = startPoint.Y + 45;

            g.DrawLine(pen, loc.X, loc.Y, loc.X + 130, loc.Y);
            g.DrawString("Clk", font, new SolidBrush(color), loc.X + 12 - g.MeasureString("Clk", font).Width / 2, loc.Y + 1);
            g.DrawString("Ratio", font, new SolidBrush(color), loc.X + 40 - g.MeasureString("Ratio", font).Width / 2, loc.Y + 1);
            g.DrawString("Ph(dg)", font, new SolidBrush(color), loc.X + 73 - g.MeasureString("Ph(dg)", font).Width / 2, loc.Y + 1);
            g.DrawString("DC(%)", font, new SolidBrush(color), loc.X + 111 - g.MeasureString("DC(%)", font).Width / 2, loc.Y + 1);
            loc.Y += 14;
            g.DrawLine(pen, loc.X, loc.Y, loc.X + 130, loc.Y);

            if (CB_c0.Checked)
            {
                g.DrawString("c0", font, new SolidBrush(color), loc.X + 12 - g.MeasureString("c0", font).Width / 2, loc.Y + 1);
                string str = nUD_c0_mul.Value + "/" + nUD_c0_div.Value;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 40 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                str = CBB_c0_phase.Text;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 73 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                str = CBB_c0_duty.Text;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 111 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                loc.Y += 14;
                g.DrawLine(pen, loc.X, loc.Y, loc.X + 130, loc.Y);
            }
            if (CB_c1.Checked)
            {
                g.DrawString("c1", font, new SolidBrush(color), loc.X + 12 - g.MeasureString("c1", font).Width / 2, loc.Y + 1);
                string str = nUD_c1_mul.Value + "/" + nUD_c1_div.Value;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 40 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                str = CBB_c1_phase.Text;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 73 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                str = CBB_c1_duty.Text;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 111 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                loc.Y += 14;
                g.DrawLine(pen, loc.X, loc.Y, loc.X + 130, loc.Y);
            }
            if (CB_e0.Checked)
            {
                g.DrawString("e0", font, new SolidBrush(color), loc.X + 12 - g.MeasureString("e0", font).Width / 2, loc.Y + 1);
                string str = nUD_e0_mul.Value + "/" + nUD_e0_div.Value;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 40 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                str = CBB_e0_phase.Text;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 73 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                str = CBB_e0_duty.Text;
                g.DrawString(str, font, new SolidBrush(color), loc.X + 111 - g.MeasureString(str, font).Width / 2, loc.Y + 1);
                loc.Y += 14;
                g.DrawLine(pen, loc.X, loc.Y, loc.X + 130, loc.Y);
            }
            g.DrawLine(pen, loc.X, startPoint.Y + 45, loc.X, loc.Y);
            loc.X += 25;
            g.DrawLine(pen, loc.X, startPoint.Y + 45, loc.X, loc.Y);
            loc.X += 30;
            g.DrawLine(pen, loc.X, startPoint.Y + 45, loc.X, loc.Y);
        ...
        */
        ...
        private void OutPut(int ok)
        {
            if (PTE)
            {
                ...
                else if (RB_e02.Checked)
                {
                    m_real = e02_m;
                }
                else if (RB_N.Checked)
                {
                    m_real = m;
                }
                */
        ...
        private bool RefreshData()
        {
            ...
            if (PTE)
            {
                ...
                for (j = 0; j < NCM; j++)
                {
                    ...
                    {
                        if (CB_c0.Checked && sort[j].c0m == 0)
                        {
                            MessageBox.Show("sort[" + j.ToString() + "].c0m==0");
                            break;
                        }
                        if (CB_c1.Checked && sort[j].c1m == 0)
                        {
                            MessageBox.Show("sort[" + j.ToString() + "].c1m==0");
                            break;
                        }
                        if (CB_e0.Checked && sort[j].e0m == 0)
                        {
                            MessageBox.Show("sort[" + j.ToString() + "].e0m==0");
                            break;
                        }
                        if (CB_c3.Checked && sort[j].c3m == 0)
                        {
                            MessageBox.Show("sort[" + j.ToString() + "].c3m==0");
                            break;
                        }
                        if (CB_c4.Checked && sort[j].c4m == 0)
                        {
                            MessageBox.Show("sort[" + j.ToString() + "].c4m==0");
                            break;
                        }
                        if (CB_e02.Checked && sort[j].e02m == 0)
                        {
                            MessageBox.Show("sort[" + j.ToString() + "].e02m==0");
                            break;
                        }
                    }
                }
                //------------ly 20160510 test end
                */
                ...
                //{
                //    LB_group.Text += "(" + sort_2[i].n.ToString() + "," + sort_2[i].m.ToString() + "," + sort_2[i].c0m.ToString() + "," + sort_2[i].c1m.ToString() + "," + sort_2[i].e0m.ToString() + "," + sort_2[i].c3m.ToString() + "," + sort_2[i].c4m.ToString() + "," + sort_2[i].e02m.ToString() + ")";
                //    count++;
                //    if (count % 6 == 0)
                //        LB_group.Text += "\n";
                //}

                //LB_group.Text += " sort_2_count = " + sort_2_count;
                ...
                if (CB_c0.Checked)
                {
                    ...
                    //LB_group.Text += "\nc0 changed [ n = " + sort_duty_c0_2[CBB_c0_duty.SelectedIndex].n.ToString()
                    //    + " m = " + sort_duty_c0_2[CBB_c0_duty.SelectedIndex].m.ToString()
                    //    + " c0m = " + sort_duty_c0_2[CBB_c0_duty.SelectedIndex].c0m.ToString()
                    //    + " c1m = " + sort_duty_c0_2[CBB_c0_duty.SelectedIndex].c1m.ToString()
                    //    + " e0m = " + sort_duty_c0_2[CBB_c0_duty.SelectedIndex].e0m.ToString() + " ]";
                }
                //c0约束结束
                ...
                if (CB_c1.Checked)
                {
                    ...
                    //LB_group.Text += "\nc1 changed [ n = " + sort_duty_c1_2[CBB_c1_duty.SelectedIndex].n.ToString()
                    //    + " m = " + sort_duty_c1_2[CBB_c1_duty.SelectedIndex].m.ToString()
                    //    + " c0m = " + sort_duty_c1_2[CBB_c1_duty.SelectedIndex].c0m.ToString()
                    //    + " c1m = " + sort_duty_c1_2[CBB_c1_duty.SelectedIndex].c1m.ToString()
                    //    + " e0m = " + sort_duty_c1_2[CBB_c1_duty.SelectedIndex].e0m.ToString() + " ]";
                }
                //c1约束结束
                ...
                if (CB_c3.Checked)
                {
                    ...
                    //LB_group.Text += "\nc3 changed [ n = " + sort_duty_c3_2[CBB_c3_duty.SelectedIndex].n.ToString()
                    //    + " m = " + sort_duty_c3_2[CBB_c3_duty.SelectedIndex].m.ToString()
                    //    + " c3m = " + sort_duty_c3_2[CBB_c3_duty.SelectedIndex].c3m.ToString()
                    //    + " c1m = " + sort_duty_c3_2[CBB_c3_duty.SelectedIndex].c1m.ToString()
                    //    + " e0m = " + sort_duty_c3_2[CBB_c3_duty.SelectedIndex].e0m.ToString() + " ]";
                }
                //c3约束结束
                ...
                if (CB_c4.Checked)
                {
                    ...
                    //LB_group.Text += "\nc4 changed [ n = " + sort_duty_c4_2[CBB_c4_duty.SelectedIndex].n.ToString()
                    //    + " m = " + sort_duty_c4_2[CBB_c4_duty.SelectedIndex].m.ToString()
                    //    + " c0m = " + sort_duty_c4_2[CBB_c4_duty.SelectedIndex].c0m.ToString()
                    //    + " c4m = " + sort_duty_c4_2[CBB_c4_duty.SelectedIndex].c4m.ToString()
                    //    + " e0m = " + sort_duty_c4_2[CBB_c4_duty.SelectedIndex].e0m.ToString() + " ]";
                }
                //c4约束结束
                ...
                if (CB_e02.Checked)
                {
                    ...
                    //LB_group.Text += "\ne02 changed [ n = " + sort_duty_e02_2[CBB_e02_duty.SelectedIndex].n.ToString()
                    //    + " m = " + sort_duty_e02_2[CBB_e02_duty.SelectedIndex].m.ToString()
                    //    + " c0m = " + sort_duty_e02_2[CBB_e02_duty.SelectedIndex].c0m.ToString()
                    //    + " c1m = " + sort_duty_e02_2[CBB_e02_duty.SelectedIndex].c1m.ToString()
                    //    + " e02m = " + sort_duty_e02_2[CBB_e02_duty.SelectedIndex].e02m.ToString() + " ]";
                }
            }
            else
            {
                if (RB_mode0.Checked)//无补偿
                {
                    ...
                    //for (i = 0; i < sort_2_count; i++)
                    //{
                    //    LB_group.Text += "(" + sort_2F[i].n.ToString() + "," + sort_2F[i].m.ToString() + "," + sort_2F[i].c0m.ToString() + "," + sort_2F[i].c1m.ToString() + "," + sort_2F[i].e0m.ToString() + ")";
                    //    count++;
                    //    if (count % 6 == 0)
                    //        LB_group.Text += "\n";
                    //}
        ...
        */
        ...
        /*
        private void nUD_c0_mul_KeyUp(object sender, KeyEventArgs e)
        {
            //c0_mul = Convert.ToInt32(nUD_c0_mul.Value);
            //saveenabled = RefreshData();
            //BT_Save.Enabled = saveenabled;
        }
        private void nUD_c1_mul_KeyUp(object sender, KeyEventArgs e)
        {
            //c1_mul = Convert.ToInt32(nUD_c1_mul.Value);
            //saveenabled = RefreshData();
            //BT_Save.Enabled = saveenabled;
        }
        private void nUD_e0_mul_KeyUp(object sender, KeyEventArgs e)
        {
            //e0_mul = Convert.ToInt32(nUD_e0_mul.Value);
            //saveenabled = RefreshData();
            //BT_Save.Enabled = saveenabled;
        }
        private void nUD_c0_div_KeyUp(object sender, KeyEventArgs e)
        {
            //c0_div = Convert.ToInt32(nUD_c0_div.Value);
            //saveenabled = RefreshData();
            //BT_Save.Enabled = saveenabled;
        }
        private void nUD_c1_div_KeyUp(object sender, KeyEventArgs e)
        {
            //c1_div = Convert.ToInt32(nUD_c1_div.Value);
            //saveenabled = RefreshData();
            //BT_Save.Enabled = saveenabled;
        }
        private void nUD_e0_div_KeyUp(object sender, KeyEventArgs e)
        {
            //e0_div = Convert.ToInt32(nUD_e0_div.Value);
            //saveenabled = RefreshData();
            //BT_Save.Enabled = saveenabled;
        ...
        */
        ...
        private void BT_Save_Click(object sender, EventArgs e)
        {
            if (bsave)
            {
                ...
                if (CB_pfd_load.Checked && RB_load.Checked)
                {
                    sw.WriteLine("//pfdena=load");
                }
                */
